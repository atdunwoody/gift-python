...
_SUMMARY_COLUMNS = (
    ...
    "HSI_cubic_root_mean",
    "HSI_cubic_root_max",
    "HSI_depth_contribution_mean",
    "HSI_velocity_contribution_mean",
    ...
    "WUA_Q_count",
    "GIFT_depth_mean_m",
    "GIFT_velocity_mean_mps",
    "GIFT_depth_at_WUA_max_m",
...
def model_reaches(
    ...
    *,
    ...
    ``HSI_mean`` and ``HSI_max`` summarize ``d.suit * v.suit * s.suit``
    over the same native discharge curve, without multiplying by wetted width.
    ``HSI_cubic_root_mean`` averages the cube root at each discharge;
    ``HSI_cubic_root_max`` is the maximum cube root. The three
    ``HSI_*_contribution_mean`` fields are exact Shapley contributions
    relative to a suitability of 1 for each component. They sum to
    ``HSI_mean - 1`` and allocate interactions equally. ``d.suit_mean`` and
    ``v.suit_mean`` are the arithmetic means of depth and velocity suitability
    across those same native discharge simulations. ``GIFT_depth_mean_m``
    and ``GIFT_velocity_mean_mps`` average modeled mean depth (``di``) and
    mean velocity (``Ui``) over those points. The ``*_at_WUA_max_*`` fields
    give the modeled hydraulics at ``WUA_Q_at_max_m3s``. These are hydraulic
    values, not habitat suitability scores. If
    ...
    for position, (index, reach) in enumerate(reaches.iterrows()):
        ...
        if any(_missing_model_input(reach[column]) for column in (
            *model_columns, *(name for name in optional_columns if name is not None),
            ...
            if output == "curves":
                null_curve = {
                    "reach_id": reach_id, "Q": np.nan,
                    "depth_m": np.nan, "velocity_mps": np.nan,
                    "d.suit": np.nan,
                    "v.suit": np.nan, "s.suit": np.nan, "w": np.nan,
                    "WUA": np.nan, "HSI": np.nan,
                    "HSI_cubic_root": np.nan,
                }
        ...
        d = curve["d.suit"].to_numpy(dtype=float)
        v = curve["v.suit"].to_numpy(dtype=float)
        s = curve["s.suit"].to_numpy(dtype=float)
        hsi = d * v * s
        curve["HSI"] = hsi
        curve["HSI_cubic_root"] = np.cbrt(hsi)
        if output == "curves":
            curve["depth_m"] = hydraulics["di"].to_numpy(dtype=float)
            curve["velocity_mps"] = hydraulics["Ui"].to_numpy(dtype=float)
            if normalize_width_col is not None:
                try:
        ...
        # Exact Shapley allocation of d*v*s - 1 at each discharge. Averaging
        # over the curve preserves the additive identity for HSI_mean.
        depth_contribution = (d - 1) * (1 / 3 + (v + s) / 6 + v * s / 3)
        velocity_contribution = (v - 1) * (1 / 3 + (d + s) / 6 + d * s / 3)
        substrate_contribution = (s - 1) * (1 / 3 + (d + v) / 6 + d * v / 3)
        ...
        max_index = int(np.flatnonzero(wua == max_wua)[0])
        q_at_max = float(q[max_index])
        ...
        summary = {
            ...
            "HSI_max": float(np.max(hsi)),
            "HSI_cubic_root_mean": float(np.mean(curve["HSI_cubic_root"])),
            "HSI_cubic_root_max": float(np.max(curve["HSI_cubic_root"])),
            "HSI_depth_contribution_mean": float(np.mean(depth_contribution)),
            "HSI_velocity_contribution_mean": float(np.mean(velocity_contribution)),
            "HSI_substrate_contribution_mean": float(np.mean(substrate_contribution)),
            "WUA_Q_min_m3s": float(np.min(q)),
            "WUA_Q_max_m3s": float(np.max(q)),
            "WUA_Q_at_max_m3s": q_at_max,
            "WUA_Q_count": len(curve),
            "GIFT_depth_mean_m": float(depth.mean()),
            "GIFT_velocity_mean_mps": float(velocity.mean()),
            "GIFT_depth_at_WUA_max_m": float(depth[max_index]),
            "GIFT_velocity_at_WUA_max_mps": float(velocity[max_index]),
    ...
    if output == "curves":
        if not outputs:
            columns = [
                "reach_id", "Q", "depth_m", "velocity_mps",
                "d.suit", "v.suit", "s.suit", "w", "WUA",
                "HSI", "HSI_cubic_root",
            ]
            if normalize_width_col is not None:
