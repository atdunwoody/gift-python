...
from gift_habitat.habitat import _nearest_suitability
...
class HabitatTests(unittest.TestCase):
    ...
    def test_sorted_lookup_matches_first_row_nearest_with_ties(self) -> None:
        targets = np.array([-1.0, 0.5, 1.0, 1.5, 2.0, 3.0])
        for values, suits in (
            (np.array([0.0, 1.0, 1.0, 2.0]), np.array([0.0, 0.4, 0.9, 1.0])),
            (np.array([2.0, 0.0, 1.0, 1.0]), np.array([1.0, 0.0, 0.4, 0.9])),
        ):
            expected = suits[np.abs(values[:, None] - targets).argmin(axis=0)]
            np.testing.assert_array_equal(
                _nearest_suitability(values, suits, targets), expected
            )
